...
class mc_dropout():
    ...
    def create_network(self):
        ...
        print("Grafo da Rede Neural criado.")
    ...
    def test(self, sess, A, X, P):
        P_mean, P_logvar, loss = sess.run([self.P_mean, self.P_logvar, self.loss],
                                               feed_dict={self.A: A, self.X: X, self.P: P})
        return P_mean, P_logvar, loss

    def predict(self, sess, A, X):
        P_mean, P_logvar = sess.run([self.P_mean, self.P_logvar], feed_dict={self.A: A, self.X: X})
        return P_mean, P_logvar

    def get_feature(self, sess, A, X, P):
        orange = sess.run(self.weights, feed_dict = {self.A: A, self.X: X, self.P: P})
        return orange
